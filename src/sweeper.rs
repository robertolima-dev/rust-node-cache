...
//! ou de chamadas manuais a `cleanupExpired()`.
//!
//! ## Ciclo de vida
//!
//! A thread recebe um [`Weak<RustCache>`], não um `Arc` forte: assim ela **não**
//! mantém o cache vivo. Se o objeto `Cache` do JS for coletado, o `Arc` some, o
//! `upgrade()` falha e a thread encerra sozinha. Além disso o [`Cache`] guarda
//! este [`Sweeper`]; quando ele é destruído (`Drop`), sinalizamos a parada e
//! fazemos `join` na thread — sem threads órfãs.
//!
//! O sono é feito em fatias curtas (não um único `sleep(interval)`) para que o
//! encerramento responda rápido ao sinal de parada, sem esperar o intervalo todo.
